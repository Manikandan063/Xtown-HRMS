      ...
      <DialogContent className="sm:max-w-[600px] rounded-[2rem] border-none shadow-2xl p-0 overflow-hidden bg-card/95 backdrop-blur-xl ring-1 ring-border">
              ...
              {uploading ? (
                ...
                <>
                  ...
                  <span className="text-[10px] text-muted-foreground opacity-60">
                    PDF, IMAGES, EXCEL, WORD, ZIP (MAX 20MB)
                  </span>
                </>
              )}
            </label>
          ...
          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? (
              ...
              files.map((file) => (
                <div
                  key={file.id}
                  className="flex items-center justify-between p-4 rounded-[1.2rem] bg-muted/30 border border-border/50 hover:border-indigo-500/30 hover:bg-white transition-all group/file"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex flex-col">
                      <span className="text-sm font-black tracking-tight line-clamp-1 max-w-[250px]">
                        {file.originalName}
                      </span>
        ...
        <DialogFooter className="p-6 bg-muted/30 border-t border-border">
          <Button
            variant="secondary"
            onClick={onClose}
            className="w-full rounded-xl font-bold uppercase tracking-widest text-[11px] h-11"
          >
            Close Manager
          </Button>
        </DialogFooter>
...
const ProjectList = () => {
  ...
  const fetchProjects = useCallback(async () => {
    try {
      ...
      const data = await apiFetch(
        `/project?page=${page}&limit=${limit}&search=${searchQuery}`,
      );
  ...
  const handleDelete = async (id) => {
    if (
      !window.confirm("Are you sure? This will hide the project from the list.")
    )
      return;
  ...
  const metrics = [
    ...
    {
      label: "On Hold",
      value: projects.filter((p) => p.projectStatus === "ON_HOLD").length,
      icon: Clock,
    ...
    <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 px-2">
        ...
        <div className="flex gap-3">
          <Button
            ...
            variant="outline"
            className="h-12 px-6 rounded-2xl flex gap-3 font-black uppercase text-[10px] tracking-widest border-border hover:bg-muted shadow-sm"
          >
            <FileDown className="h-4 w-4" /> Report
          </Button>
          {canEdit && <ProjectModal onSuccess={fetchProjects} />}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {metrics.map((m, i) => (
          <Card
            key={i}
            className="border-none shadow-xl p-6 rounded-3xl bg-card/70 backdrop-blur-md hover:shadow-2xl transition-all group ring-1 ring-border"
          >
            <div className="flex items-center gap-5">
              <div
                className={`h-14 w-14 rounded-2xl ${m.bg} flex items-center justify-center ${m.color} group-hover:rotate-6 transition-transform`}
              >
      ...
      <Card className="border-none shadow-2xl rounded-[2rem] overflow-hidden bg-card/50 backdrop-blur-xl ring-1 ring-border">
        <CardHeader className="bg-muted/50 border-b border-border px-8 py-6">
          <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
            <CardTitle className="text-xl font-bold tracking-tight italic">
              All Projects
            </CardTitle>
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="font-bold py-6 pl-8 uppercase text-[10px] tracking-widest text-muted-foreground">
                  Project Name
                </TableHead>
                <TableHead className="font-bold py-6 uppercase text-[10px] tracking-widest text-muted-foreground">
                  Dates
                </TableHead>
                <TableHead className="font-bold py-6 uppercase text-[10px] tracking-widest text-muted-foreground">
                  Progress
                </TableHead>
                <TableHead className="font-bold py-6 text-center uppercase text-[10px] tracking-widest text-muted-foreground">
                  Status
                </TableHead>
                <TableHead className="font-bold py-6 text-right pr-8 uppercase text-[10px] tracking-widest text-muted-foreground">
                  More
                </TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredProjects.length === 0 ? (
                <TableRow>
                  <TableCell
                    colSpan={5}
                    className="text-center py-20 text-muted-foreground italic font-medium opacity-50 uppercase tracking-tighter"
                  >
                    No projects found.
                  </TableCell>
                </TableRow>
                ...
                filteredProjects.map((prj) => (
                  <TableRow
                    key={prj.id}
                    className="hover:bg-primary/5 border-b border-border transition-all group"
                  >
                    <TableCell className="py-6 pl-8">
                      <div className="space-y-1">
                        ...
                        <p className="text-[11px] text-muted-foreground font-medium italic opacity-70 line-clamp-1 max-w-[250px]">
                          {prj.description || "No description added."}
                        </p>
                      </div>
                    </TableCell>
                    <TableCell className="py-6 font-bold text-[10px] text-muted-foreground uppercase tracking-widest overflow-hidden">
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex flex-col">
                            <span className="text-[8px] text-emerald-600/50 font-black">
                              START PHASE
                            </span>
                        ...
                        >
                          ...
                          <div className="flex flex-col">
                            <span className="text-[8px] text-rose-600/50 font-black">
                              TARGET DEADLINE
                            </span>
                            <span className="text-foreground font-black tracking-tighter text-xs">
                              {prj.endDate
                    ...
                    <TableCell className="text-center py-6">
                      <Badge
                        className={`rounded-xl px-4 py-1.5 font-black text-[10px] uppercase shadow-none border-none ${getStatusColor(prj.projectStatus)}`}
                      >
                        ...
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="group-hover:bg-card rounded-full h-10 w-10 shadow-sm border border-transparent hover:border-border transition-all"
                            >
                          ...
                          <DropdownMenuContent
                            align="end"
                            className="w-56 rounded-[1.2rem] p-2 bg-card backdrop-blur-xl border border-border shadow-2xl"
                          >
                            ...
                            <ProjectTeamModal
                              ...
                              trigger={
                                <DropdownMenuItem
                                  ...
                                  className="gap-3 py-3 rounded-xl focus:bg-muted cursor-pointer font-bold text-xs uppercase transition-all"
                                >
                            ...
                            <ProjectProgressModal
                              ...
                              trigger={
                                <DropdownMenuItem
                                  onSelect={(e) => e.preventDefault()}
                                  className="gap-3 py-3 rounded-xl focus:bg-indigo-500/10 text-indigo-500 cursor-pointer font-bold text-xs uppercase transition-all"
                                >
